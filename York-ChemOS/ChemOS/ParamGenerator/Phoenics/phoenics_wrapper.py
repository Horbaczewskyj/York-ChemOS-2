...
class PhoenicsWrapper(OptimizerWrapper):
	...
	def _create_config_file(self, identifier, exp_settings):
		...
		for variable in exp_settings['variables']:
			...
			new_variable += ' "low": %s, '    % str(variable['low'])
			new_variable += ' "high": %s, '   % str(variable['high'])
		...
		# create the objective list - we need the objective specifications
		new_objective = ''
		for objective in exp_settings['objectives']:
			new_objective += '{"%s_%s": {' % (objective['name'], objective['operation'])
			new_objective += '"hierarchy": %s, ' % objective['hierarchy']
			new_objective += '"type": "minimum", '
			new_objective += '"tolerance": %s} }' % objective['tolerance']
			new_objective += ', '
		new_objective = new_objective[:-2]
		replace_dict['{@OBJECTIVE_LIST}'] = new_objective
...
#	def __enter__(identifier, exp_settings):
